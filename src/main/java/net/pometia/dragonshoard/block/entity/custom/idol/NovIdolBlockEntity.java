...
public class NovIdolBlockEntity extends BlockEntity {
    ...
    public NovIdolBlockEntity(BlockPos pos, BlockState blockState) {
        super(ModBlockEntities.NOV_IDOL_BE.get(), pos, blockState);
    };
    ...
    public static void clientTick (Level level, BlockPos pos, BlockState state, NovIdolBlockEntity blockEntity){
    ...
    public static void serverTick (Level level, BlockPos pos, BlockState state, NovIdolBlockEntity blockEntity){
    ...
    private static void updateHunting (NovIdolBlockEntity blockEntity, List < BlockPos > enhancingBlocks){
    ...
    private static void updateDestroyTarget (Level level, BlockPos pos, BlockState
            state, List < BlockPos > enhancingBlocks, NovIdolBlockEntity blockEntity){
    ...
    private static void updateClientTarget (Level level, BlockPos pos, NovIdolBlockEntity blockEntity){
        ...
        for (int i = -4; i <= 4; i++) {
            for (int j = -4; j <= 4; j++) {
                for (int kx = -4; kx <= 4; kx++) {
                    ...
                    if (blockState.is(ModTags.Blocks.NOV_FAVORED_BLOCKS_VALUE_1)) {
        ...
        for (int i = -4; i <= 4; i++) {
            for (int j = -4; j <= 4; j++) {
                for (int kx = -4; kx <= 4; kx++) {
                    ...
                    if (blockState.is(ModTags.Blocks.NOV_FAVORED_BLOCKS_VALUE_2)) {
                        enhancingBlocks.add(blockPos2);
                        enhancingBlocksVal2.add(blockPos2);
                    }

                    if (blockState.is(ModTags.Blocks.NOV_FAVORED_BLOCKS_VALUE_3)) {
                        enhancingBlocks.add(blockPos2);
                        enhancingBlocksVal3.add(blockPos2);
                    }
                    if (blockState.is(ModTags.Blocks.NOV_FAVORED_BLOCKS_VALUE_4)) {
                        enhancingBlocks.add(blockPos2);
                        enhancingBlocksVal4.add(blockPos2);
                    }
                    if (blockState.is(ModTags.Blocks.NOV_FAVORED_BLOCKS_VALUE_5)) {
                        enhancingBlocks.add(blockPos2);
