...
public class JunIdolBlockEntity extends BlockEntity {
    ...
    public JunIdolBlockEntity(BlockPos pos, BlockState blockState) {
        super(ModBlockEntities.JUN_IDOL_BE.get(), pos, blockState);
    };
    ...
    public static void clientTick (Level level, BlockPos pos, BlockState state, JunIdolBlockEntity blockEntity){
    ...
    public static void serverTick (Level level, BlockPos pos, BlockState state, JunIdolBlockEntity blockEntity){
    ...
    private static void updateHunting (JunIdolBlockEntity blockEntity, List < BlockPos > enhancingBlocks){
    ...
    private static void updateDestroyTarget (Level level, BlockPos pos, BlockState
            state, List < BlockPos > enhancingBlocks, JunIdolBlockEntity blockEntity){
    ...
    private static void updateClientTarget (Level level, BlockPos pos, JunIdolBlockEntity blockEntity){
        ...
        for (int i = -4; i <= 4; i++) {
            for (int j = -4; j <= 4; j++) {
                for (int kx = -4; kx <= 4; kx++) {
                    ...
                    if (blockState.is(ModTags.Blocks.JUN_FAVORED_BLOCKS_VALUE_1)) {
        ...
        for (int i = -4; i <= 4; i++) {
            for (int j = -4; j <= 4; j++) {
                for (int kx = -4; kx <= 4; kx++) {
                    ...
                    if (blockState.is(ModTags.Blocks.JUN_FAVORED_BLOCKS_VALUE_2)) {
                        enhancingBlocks.add(blockPos2);
                        enhancingBlocksVal2.add(blockPos2);
                    }

                    if (blockState.is(ModTags.Blocks.JUN_FAVORED_BLOCKS_VALUE_3)) {
                        enhancingBlocks.add(blockPos2);
                        enhancingBlocksVal3.add(blockPos2);
                    }
                    if (blockState.is(ModTags.Blocks.JUN_FAVORED_BLOCKS_VALUE_4)) {
                        enhancingBlocks.add(blockPos2);
                        enhancingBlocksVal4.add(blockPos2);
                    }
                    if (blockState.is(ModTags.Blocks.JUN_FAVORED_BLOCKS_VALUE_5)) {
                        enhancingBlocks.add(blockPos2);
